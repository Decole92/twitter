...
import { EllipsisHorizontalIcon, ChatBubbleOvalLeftIcon, HeartIcon, ShareIcon, ArrowUpTrayIcon, TrashIcon, ArrowPathIcon, ArrowLeftOnRectangleIcon} from '@heroicons/react/24/outline'
...
useEffect(() => onSnapshot(collection(db, "posts", orginalPostId, "comments", commentId, "likes"),
(snapshot) => setLikesComment(snapshot.docs))
 
, [db], commentId, orginalPostId)

useEffect(()=>{ setHasLikedComment(likesComment.findIndex((like) => like.id === session?.user?.uid) !== -1)
},[likesComment])
...
<div className="hover:bg-gray-50 flex p-3 cursor-pointer border-b border-gray-200">
      ...
      <div className="flex-1">
       ...
       <p className="-mt-2 mb-1 text-gray-500">Replying to <span className="text-twitter">@{post?.data()?.name}</span></p>
    ...
    <img
        ...
        {
         hasLikedComment? <FilledIcon onClick={()=>likeComment()} className="text-red-500 h-9 p-2"/> : <HeartIcon onClick={()=>likeComment()} className="h-9 p-2 hover:bg-red-100 hover:text-red-500 rounded-full cursor-pointer transition-all duration-60" />
         
          }
        {likesComment.length > 0 && <span className={`${hasLikedComment && "text-red-400"}`}>{likesComment.length}</span> }

          </div>


         {
          comment?.data()?.id === session?.user.uid &&  <TrashIcon onClick={()=>deleteComment()}
