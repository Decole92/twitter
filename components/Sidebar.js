...
export default function Sidebar() {
  ...
  return (
    <div className="hidden sm:flex flex-col p-2 xl:items-start fixed h-full xl:ml-24">
      {/* Twitter Logo */}
      <img
        className="h-10 w-10 ml-4 mb-5 hover:bg-blue-100 rounded-full p-1 cursor-pointer transition-all duration-200"
      ...
      <div className="py-10 ">
        <img
          className="lg:hidden lg:inline-flex cursor-pointer"
          width="100"
          height="100"
          src="https://thumbs.dreamstime.com/b/add-tweet-post-button-icon-vector-twitter-social-media-element-219099895.jpg"
        />

        <button className="hidden xl:inline mx-5 bg-twitter p-2 text-white font-bold w-56 h-12 rounded-full">
          Tweet
        </button>
      </div>
      ...
      <div className="flex transiton-all p-2 absolute bottom-5 duration-200 hover:bg-gray-100 rounded-full cursor-pointer">
        <img
          className="w-12 h-12 rounded-full mx-5 items-center"
          onClick={() => signOut()}
          alt=""
          src={session?.user.image}
        />
        <div className="px-1 hidden xl:inline">
