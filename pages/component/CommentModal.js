...
<h4 className="font-bold text-[15px] sm:text-[16px] hover:underline">
   ...
   <p className="text-black text-[15px] sm:text-[16px] -mt-4 ml-[58px]">{post?.data()?.text}</p>

   <p className="text-gray-500 -[15px] sm:text-[16px]  ml-[58px]">Replying to <span className="text-twitter">@{post?.data()?.username}</span></p>

<div  className="flex pl-2.5 mt-5">

<img className=" w-11 h-11 rounded-full" alt="user-photo" src={session?.user.image} />

<div className="flex flex-1 items-center pl-2">
  <div className="flex flex-1 flex-col">
    <input value={input}  onChange={(e) => setInput(e.target.value)} type="text" className=" -mt-4 h-20 w-full text-xl outline-none placeholder:text-xl" placeholder="Tweet your reply"/>
...
<div className="flex  items-center px-3">
     ...
     <PhotoIcon   onClick={()=>filePickerRef.current.click()} className=" hidden h-7 w-7 cursor-pointer hover:bg-blue-200 rounded-full p-1"/>
