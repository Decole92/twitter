...
useEffect(
  () => onSnapshot(collection(db, "posts", id, "comments"), (snapshot) => setComment(snapshot.docs))
...
const likePost = async() => {
  ...
  return (
    ...
    <img
        ...
        {
         hasLiked ? <FilledIcon onClick={()=>likePost()} className="text-red-500 h-9 p-2"/> : <HeartIcon onClick={()=>likePost()} className="h-9 p-2 hover:bg-red-100 hover:text-red-500 rounded-full cursor-pointer transition-all duration-60" />
         
          }
        {likes.length > 0 && <span className={`${hasLiked && "text-red-400"}`}>{likes.length}</span> }

          </div>


         {
          post?.data()?.id === session?.user.uid &&  <TrashIcon onClick={()=>deletePost()}
